...
import {
  ...
  // Use the same portfolio items for now, but these could be replaced with specific ones later
...
const Portfolio = ({ page }) => {
  ...
  // For now, use web and app portfolios for all services
  // This could be expanded with specific portfolios for each service type
  const portfolio = isWeb ? webDevelopmentPortfolio : appDevelopmentPortfolio;
  return (
    <div className="wrapper py-10 space-y-4">
      <div className="flex flex-col items-center">
        <SubHeading
          heading={`${isWeb ? "Web" : "App"} Portfolio`}
          className="text-white"
        />
        <h2 data-aos="fade-up" className="text1 text-center">
          Our {isWeb ? "Web" : "App"} Development Projects
        </h2>
      </div>
      <div
        data-aos="fade-up"
            ...
            <div className="absolute inset-0 bg-primary/90 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300 z-[3]">
              <div className="text-center p-6">
                ...
                <Link
                  ...
                  className="px-4 py-2 bg-white text-primary rounded-full font-medium hover:bg-secondary hover:text-black transition-all duration-300"
                >
                  View Project
                </Link>
              </div>
